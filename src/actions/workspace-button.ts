...
function colorSvg(hex: string, opts?: { selected?: boolean; unread?: boolean }): string {
  let overlay = "";
  if (opts?.selected) {
    // White border ring to indicate selected workspace
    overlay = `<rect x="3" y="3" width="138" height="138" rx="8" fill="none" stroke="#FFFFFF" stroke-width="6"/>`;
  }
  if (opts?.unread) {
    // Orange dot in top-right corner to indicate needs input
